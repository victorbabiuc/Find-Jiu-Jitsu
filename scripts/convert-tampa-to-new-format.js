...
// Alternative: Use Node.js built-in CSV parsing for Tampa
function parseTampaCSVLine(line) {
  // For Tampa CSV, we know the structure and can handle it manually
  ...
  // id, name, address, website, distance, matFee, dropInFee, sessionDay, sessionTime, sessionType (10 fields)
  for (let j = 0; j < 10; j++) {
    result.push(parts[i++]);
  }
  ...
  // last_updated (1 field)
  for (let j = 0; j < 1 && i < parts.length; j++) {
    result.push(parts[i++]);
  }
...
function convertTampaToNewFormat() {
  console.log('🔄 Converting Tampa gyms to new format...');
  
  // Read the original CSV
  const csvData = fs.readFileSync('data/tampa-gyms.csv', 'utf8');
  ...
  // Write to new file
  const outputPath = 'data/tampa-gyms-new-format.csv';
...
module.exports = { convertTampaToNewFormat }; 