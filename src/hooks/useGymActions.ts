...
export const useGymActions = ({ favorites, toggleFavorite, shareCardRef }: UseGymActionsProps) => {
  ...
  const handleCopyGymWithState = useCallback(async (gym: OpenMat) => {
  ...
  const handleShareGym = useCallback(async (gym: OpenMat, session?: OpenMatSession) => {
    ...
    try {
      const firstSession = session || (gym.openMats && gym.openMats.length > 0 ? gym.openMats[0] : null);
  ...
  const handleToggleFavorite = useCallback((gym: OpenMat) => {
    haptics.light();
