...
const ResultsScreen: React.FC<ResultsScreenProps> = ({ route }) => {
  ...
  const [selectedLocationType, setSelectedLocationType] = useState<
    'tampa-downtown' | 'current-location' | 'search-address'
  >('tampa-downtown');
  ...
  useEffect(() => {
    const runEntranceAnimations = async () => {
      ...
      animations
        .stagger([headerAnim, filterAnim, listAnim], (value, index) =>
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const city = location.toLowerCase().includes('austin')
          ? 'austin'
          : location.toLowerCase().includes('miami')
            ? 'miami'
            : location.toLowerCase().includes('st. petersburg')
              ? 'stpete'
              : location.toLowerCase().includes('tampa')
                ? 'tampa'
                : 'tampa';
        ...
        logger.success('ResultsScreen: Data loaded successfully -', {
          count: data.length,
          firstGym: data[0],
        });
  ...
  const getFeeDisplay = (matFee: number, dropInFee?: number) => {
    // Open Mat Fee logic
    const openMatText =
      matFee === 0 ? 'Open Mat - Free' : matFee > 0 ? `Open Mat - $${matFee}` : 'Open Mat - ?';

    const openMatColor = matFee === 0 ? '#10B981' : matFee > 0 ? '#111518' : '#9CA3AF';

    // Drop in Fee logic
    const dropInText =
      dropInFee === 0
        ? 'Drop in - Free'
        : dropInFee && dropInFee > 0
          ? `Drop in - $${dropInFee}`
          : 'Drop in - ?';

    const dropInColor =
      dropInFee === 0 ? '#10B981' : dropInFee && dropInFee > 0 ? '#111518' : '#9CA3AF';
  ...
  const getEmptyStateMessage = (): { title: string; subtitle: string; icon: string } => {
    ...
    if (
      (hasRadius && hasFree) ||
      (hasRadius && (hasGi || hasNoGi)) ||
      (hasFree && (hasGi || hasNoGi))
    ) {
      return {
  ...
  const shortenAddress = (fullAddress: string): string => {
    if (
      !fullAddress ||
      fullAddress === 'Tampa Downtown' ||
      fullAddress === 'Current Location' ||
      fullAddress === 'Search Address'
    ) {
      return fullAddress;
    }
  ...
  const handleLocationSelect = async (
    locationType: 'tampa-downtown' | 'current-location' | 'search-address'
  ) => {
    ...
    switch (locationType) {
      case 'tampa-downtown':
        ...
        switch (location.toLowerCase()) {
          ...
          case 'st-pete':
          case 'st. petersburg':
            setUserLocation({ latitude: 27.7731, longitude: -82.64 });
            break;
          case 'tampa':
          default:
  ...
  const handleGetCurrentLocation = async () => {
    try {
      ...
      logger.error('GPS location error:', {
  ...
  const groupGymsByLocation = (gyms: OpenMat[]): OpenMat[] => {
    ...
    const grouped = gyms.reduce((acc, gym) => {
      ...
      if (existing) {
        ...
        logger.group(`Combining sessions for ${gym.name}`, {
          newSessions: gym.openMats?.length || 0,
        });
  ...
  const sortedGyms = useMemo(() => {
    ...
    logger.sort(
      'Sorted gyms by next session:',
      sorted.slice(0, 3).map((gym, index) => ({
        rank: index + 1,
        name: gym.name,
        sessions: gym.openMats.length,
        sessionDetails: gym.openMats.map(
          session => `${session.day} ${session.time} (${session.type})`
        ),
      }))
    );
  ...
  const filteredGyms = useMemo(() => {
    ...
    const hasActiveFilters =
      activeFilters.gi || activeFilters.nogi || activeFilters.price === 'free';
      ...
      filtered = filtered
        ...
        .map(gym => {
          ...
          if (activeFilters.gi && !activeFilters.nogi) {
            ...
            filteredSessions = gym.openMats.filter(
              session =>
      ...
      <View style={styles.headerContainer}>
        ...
        <Animated.View style={[styles.locationPickerSection, { opacity: headerAnim }]}>
          ...
          >
            ...
            <Text style={styles.locationPickerText} numberOfLines={1} ellipsizeMode="tail">
              Near: {getLocationDisplayName(location)} ▼
            </Text>
          </TouchableOpacity>
        </Animated.View>
              ...
              <View style={styles.locationPickerModalContent}>
                <View style={styles.locationPickerModalHeader}>
                  ...
                  <TouchableOpacity
                    ...
                    style={styles.locationPickerModalCloseButton}
                  >
                ...
                <TouchableOpacity
                  style={[
                    ...
                    selectedLocationType === 'tampa-downtown' &&
                      styles.locationPickerOptionSelected,
                  ]}
                ...
                >
                  ...
                  <Text
                    style={[
                      ...
                      selectedLocationType === 'tampa-downtown' &&
                        styles.locationPickerOptionTextSelected,
                    ]}
                  >
                    Tampa Downtown
                  </Text>
                  ...
                  <TouchableOpacity
                    style={[
                      ...
                      (!searchQuery.trim() || isSearching) &&
                        styles.searchModalSearchButtonDisabled,
                    ]}
